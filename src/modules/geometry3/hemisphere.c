    ...
    float radius)
{
    ...
    ecs_entity_t asset = flecsEngine_findCachedAsset(&ctx->hemisphere_cache, key);
    if (asset) {
        return asset;
    }
    ...
    ecs_iter_t *it)
{
    ...
    for (int32_t i = 0; i < it->count; i ++) {
        ...
        ecs_entity_t old_asset = flecsEngine_findCachedAsset(&ctx->hemisphere_cache, old_key);
        if (old_asset) {
