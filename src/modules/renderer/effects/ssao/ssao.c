...
static const char *kShaderSource =
    ...
    "  let vp_l = reconstruct_view_pos(\n"
    "    in.uv + vec2<f32>(-px, 0.0),\n"
    "    textureLoad(depth_texture, texel + vec2<i32>(-1, 0), 0));\n"
    "  let vp_u = reconstruct_view_pos(\n"
    "    in.uv + vec2<f32>(0.0, -py),\n"
    "    textureLoad(depth_texture, texel + vec2<i32>(0, -1), 0));\n"
    "  let vp_d = reconstruct_view_pos(\n"
    "    in.uv + vec2<f32>(0.0, py),\n"
    "    textureLoad(depth_texture, texel + vec2<i32>(0, 1), 0));\n"
    "  let dx_fwd = vp_r - view_pos;\n"
    "  let dx_bwd = view_pos - vp_l;\n"
    "  let dy_fwd = vp_u - view_pos;\n"
    "  let dy_bwd = view_pos - vp_d;\n"
    "  let dx = select(dx_fwd, dx_bwd, abs(dx_bwd.z) < abs(dx_fwd.z));\n"
    "  let dy = select(dy_fwd, dy_bwd, abs(dy_bwd.z) < abs(dy_fwd.z));\n"
