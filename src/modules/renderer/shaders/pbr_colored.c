...
/* Non-transmission PBR shader. Reads per-instance material data from the
 * storage buffer at @group(2) @binding(0), and samples PBR texture arrays
 * at @group(1). Each texture sample is gated on the material's layer
 * index — a layer of 0 (the reserved neutral slot) means "no texture
 * assigned", so the material's factor values drive the shading directly. */
static const char *kShaderSource =
     ...
     * material's color factor directly. */
     ...
     * specify an emissive color but has non-zero strength. */
