...
static const char *kShaderSource =
    ...
    "  let tb_max = max(dot(T, T), dot(B, B));\n"
    "  let invmax = inverseSqrt(max(tb_max, 1e-10));\n"
    "  let tbn = mat3x3<f32>(T * invmax, B * invmax, N);\n"
    "  let tbn_normal = normalize(tbn * tangent_normal);\n"
    "  let mapped_normal = select(tbn_normal, N, tb_max < 1e-10);\n"
    "\n"
